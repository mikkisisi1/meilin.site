...
/**
 * Slim You — B5: Post-questionnaire auth prompt.
 * Shown once after the intake is completed (only for guests).
 * 4 options: Google / Apple / WhatsApp / Continue as Guest.
 * Apple & WhatsApp are placeholders ("Coming soon"); Google routes to /auth (existing email auth screen).
 */
export default function AuthPromptModal({ open, onClose }) {
  ...
  return (
    <div className="xc-auth-prompt-overlay" onClick={onClose} data-testid="auth-prompt-modal">
      <div className="xc-auth-prompt-modal" onClick={(e) => e.stopPropagation()}>
        <button type="button" onClick={onClose} className="xc-auth-prompt-close" data-testid="auth-prompt-close-btn">
